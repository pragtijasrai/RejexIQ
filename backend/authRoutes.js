...
async function findOrCreateGoogleUser({ name, email, avatar, uid }) {
  ...
  if (u) { u.isNewUser = false; return u; }
  u = {
    _id: "g_" + Date.now(),
    ...
    onboarded: false, school: "", branch: "", username: "",
    track: "", trackSelected: false, isNewUser: true,
    createdAt: new Date().toISOString(),
    toPublic() {
...
router.post("/signin", async (req, res, next) => {
  try {
    ...
    if (user.provider === "google") return res.status(400).json({ error: "This account uses Google Sign-In. Please continue with Google." });
...
router.post("/google", async (req, res, next) => {
  try {
    ...
    if (!email) return res.status(400).json({ error: "Google auth data missing" });
